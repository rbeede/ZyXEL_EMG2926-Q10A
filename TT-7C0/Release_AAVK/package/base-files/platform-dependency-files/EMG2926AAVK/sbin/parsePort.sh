...
echo -n "$ftp_port $www_port $telnet_port $https_port $ssh_port" > /tmp/system_port
#echo -n "$ftp_port $www_port $telnet_port $https_port $ssh_port" > /tmp/system_using_port
...
do
	...
	local proto=$(uci get nat.rule$i.protocol)
	[ "$proto" == "udp" ] && continue
		...
		if [ $(echo $j |grep "-") ];then
			if [ $(echo $j | awk -F '-' '{print $1}') -le $(echo $j | awk -F '-' '{print $2}') ];then
				...
				do	
					...
					#echo -n " $k" >> /tmp/system_using_port
				done
			else 	
			...
			#echo -n " $j" >> /tmp/system_using_port
		fi
