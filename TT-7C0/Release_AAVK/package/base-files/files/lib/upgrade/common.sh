...
run_ramfs_config() { # <command> [...]
	install_bin /bin/busybox /bin/ash /bin/sh /bin/mount /bin/umount        \
		/sbin/pivot_root /usr/bin/wget /sbin/reboot /bin/sync /bin/dd   \
		/bin/grep /bin/cp /bin/mv /bin/tar /usr/bin/md5sum "/usr/bin/[" \
		/bin/vi /bin/ls /bin/cat /usr/bin/awk /usr/bin/hexdump          \
		/bin/sleep /bin/zcat /usr/bin/bzcat /usr/bin/printf /usr/bin/wc /usr/sbin/flash_eraseall /usr/sbin/nandwrite
	...
	umount -l /dev/mtdblock4
	umount -l /
	sleep 2

	mtd erase /dev/mtd4
	mtd write /tmp/rootfs_data.configCk /dev/mtd4 
