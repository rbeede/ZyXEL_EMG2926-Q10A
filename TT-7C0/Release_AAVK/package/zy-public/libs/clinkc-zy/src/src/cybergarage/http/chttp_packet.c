...
#if 0
	else if ( (cg_http_packet_getheadervalue(httpPkt,CG_HTTP_CONTENT_LENGTH) == NULL) &&  
		(cg_http_packet_getheadervalue(httpPkt,CG_HTTP_SERVER) == NULL) )
#else /*Why need to check Server name above? */
	else if (cg_http_packet_getheadervalue(httpPkt,CG_HTTP_CONTENT_LENGTH) == NULL)
#endif
	{
...
#if 1 /* Fix for HFS Download : when the return is 0 of read, it means the close of this session */
		if(readLen == 0)
			return FALSE;
#endif
	}
