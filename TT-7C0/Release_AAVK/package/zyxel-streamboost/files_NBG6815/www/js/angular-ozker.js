...
angular.module('ozker', ['poll', 'rate','stats'])
  ...
  .service("nodes", ['$http', 'rateCalculator', 'router', 'flows', function($http, rateCalculator, router, flows) {
    ...
    function updateNode(node, data) {
      ...
      node.up_rate = node.upRateCalculator.calculate(node.timestamp, node.up, router.model.traffic.up_limit);
      node.down_rate = node.downRateCalculator.calculate(node.timestamp, node.down, router.model.traffic.down_limit);
    }
    ...
    self.fetch = function() {
      ...
      promise = promise.then(function(response) {
        ...
        router.model.traffic.up_rate = router.upRateCalculator.calculate(router.model.traffic.timestamp, router.model.traffic.up, router.model.traffic.up_limit);
        router.model.traffic.down_rate = router.downRateCalculator.calculate(router.model.traffic.timestamp, router.model.traffic.down, router.model.traffic.down_limit);
        return self.model;
      });
      return promise;
    }
  ...
  .service("flows", ['$http', 'rateCalculator', 'router', function($http, rateCalculator, router) {
    ...
    function updateCurrentRate(flow, data) {
      ...
      flow.up_rate = flow.upRateCalculator.calculate(flow.timestamp, flow.up, router.model.traffic.up_limit);
      flow.down_rate = flow.downRateCalculator.calculate(flow.timestamp, flow.down, router.model.traffic.down_limit);
    }
