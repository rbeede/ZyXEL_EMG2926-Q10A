                    ...
                    nHeight)
{
    ...
    if(bbox != null)
    {
        ...
        var rect = graph.append("rect")
            ...
            .attr("x", nItemX)
            .attr("y", nItemY)
            .attr("width", nHeight * nRatioWidth)
            .attr("height", nHeight)
            .style("fill", rgb);
        ...
        var nStartX = nItemX + nHeight * nRatioWidth - 10;
...
function getNumberOfFlowEvents()
{
    var rtData = null;

    $.ajax({
        url: '/cgi-bin/ozkerz?numbOfCurrentEventFlows=1',
        type: "GET",
        async: false,
        dataType: "json",
        cache: false,
...
*/
function getHourData_bak( rtData,
                      mac,
                      nMonth,
                      nDay,
                      callback)
{
    ...
    for(var x in eData)
    {
        ...
        if(typeof eData[x].openTime == 'undefined')
        {
            // console.log('eData[x] has no openTime:');
            // console.log(eData[x]);
            eData[x].openTime = new Date(0);
        }
        ...
        {
            // console.log('eData[x] has no closeTime:');
            // console.log(eData[x]);

            eData[x].closeTime   = new Date(now);
            // // openTime + 1 hour.
            // eData[x].closeTime = new Date(eData[x].openTime);
            // eData[x].closeTime.setHours(eData[x].closeTime.getHours() + 1);

            // console.log('eData[x] after added one hour to  closeTime:');
            // console.log(eData[x]);
        ...
        // console.log('getHours, hour: ' + hour + ', result, UUID: ' + x + ', policy_id: ' + eData[x].policy_id + ', rx_bytes: ' + eData[x].rx_bytes + ', tx_bytes: ' + eData[x].tx_bytes + ', mac: ' + eData[x].mac + ', openTime: ' + eData[x].openTime + ', closeTime: ' + eData[x].closeTime );
                      ...
                      callback)
{
    ...
    for(var x in rtData)
    {
        ...
        else if(rtData[x].event == "milestone" || rtData[x].event == "close") //is this a close / milestone update event?
        {
            ...
            var val = parseInt(rtData[x].details.rx_bytes);
            eData[uuid].checkPoints[utcTime].rx_bytes = isNaN(val) ? 0 : val;
            val = parseInt(rtData[x].details.tx_bytes);
            eData[uuid].checkPoints[utcTime].tx_bytes = isNaN(val) ? 0 : val;
