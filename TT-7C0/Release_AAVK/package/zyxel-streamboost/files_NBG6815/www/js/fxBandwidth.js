...
function bwTest(strULEdit,strDLEdit)
{
    ...
    $.ajax(
    {
	    ...
	    alert("Server unreachable, check Internet connection");
        }
    });
}
    ...
    else
    {
        ...
        else if(nDnBW != NaN && nUpBW != NaN) //legit numbers?
        {
            ...
            if(nDnBW!=-1 && nUpBW !=-1 && nDnBW!=0 && nUpBW !=0)
            {
                ...
                nnDnBW = eval(nDnBW * 8.0 / 1000000);//Bytes to Mbps
                nnUpBW = eval(nUpBW * 8.0 / 1000000);
                DnBW = Math.round(nnDnBW * 100)/ 100;
                UpBW = Math.round(nnUpBW * 100)/ 100;
				
                //if we have valid values put them in the text field
                document.General.StreamboostUp.value = UpBW;
                document.General.StreamboostDown.value = DnBW;
