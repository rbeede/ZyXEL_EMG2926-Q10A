...
static void qca955x_set_speed_xmii(int speed)
{
	...
	pdata = ath79_eth0_device.dev.platform_data;
	val = ath79_get_eth_pll(pdata->phy_if_mode == PHY_INTERFACE_MODE_SGMII?1:0, speed);

	base = ioremap_nocache(AR71XX_PLL_BASE, AR71XX_PLL_SIZE);
	__raw_writel(val, base + QCA955X_PLL_ETH_XMII_CONTROL_REG);
	iounmap(base);
	//printk("qca955x_set_speed_xmii: QCA955X_PLL_ETH_XMII_CONTROL_REG=0x%08X\n", val);
}
...
static void qca955x_set_speed_sgmii(int speed)
{
	...
	pdata = ath79_eth0_device.dev.platform_data;
	val = ath79_get_eth_pll(pdata->phy_if_mode == PHY_INTERFACE_MODE_SGMII?0:1, speed);

	base = ioremap_nocache(AR71XX_PLL_BASE, AR71XX_PLL_SIZE);
	__raw_writel(val, base + QCA955X_PLL_ETH_SGMII_CONTROL_REG);
	iounmap(base);
	//printk("qca955x_set_speed_sgmii: QCA955X_PLL_ETH_SGMII_CONTROL_REG=0x%08X\n", val);
}
